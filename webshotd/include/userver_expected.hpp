...
[[nodiscard]] Expected<T, E> parse(std::string_view jsonText, G &&mapError)
{
    return catchException<::json::Exception, E>(
        [&]() { return ::json::FromString(jsonText).template As<T>(); }, std::forward<G>(mapError)
    );
}

template <typename T, typename E>
    requires std::copy_constructible<E>
[[nodiscard]] Expected<T, E> parse(std::string_view jsonText, E error)
{
    return catchException<::json::Exception, E>(
        [&]() { return ::json::FromString(jsonText).template As<T>(); }, std::move(error)
    );
}
...
template <typename E, typename G>
    requires std::invocable<G, const us::utils::TracefulException &> &&
             std::constructible_from<
                 E, std::invoke_result_t<G, const us::utils::TracefulException &>>
[[nodiscard]] Expected<std::string, E> stringify(::json::Value value, G &&mapError)
{
    return catchUserver<E>(
...
template <typename E>
    requires std::copy_constructible<E>
[[nodiscard]] Expected<std::string, E> stringify(::json::Value value, E error)
{
    return catchUserver<E>([&]() { return ::json::ToString(std::move(value)); }, std::move(error));
}

template <typename T, typename E, typename G>
    requires std::copy_constructible<std::remove_cvref_t<G>>
