...
in {
  ...
  tasks."webshot:devUp" = {
    exec = "bash containers/process-compose/webshot_pc.sh dev up";
    cwd = config.git.root;
  };

  tasks."webshot:devDown" = {
    exec = "bash containers/process-compose/webshot_pc.sh dev down";
    cwd = config.git.root;
  };

  tasks."webshot:devStatus" = {
    exec = "bash containers/process-compose/webshot_pc.sh dev status";
    cwd = config.git.root;
  };

  tasks."webshot:devLogs" = {
    exec = "bash containers/process-compose/webshot_pc.sh dev logs";
    cwd = config.git.root;
  };
  ...
  tasks."webshot:prodlikeUp" = {
    exec = "bash containers/process-compose/webshot_pc.sh prodlike up";
    cwd = config.git.root;
  };

  tasks."webshot:prodlikeDown" = {
    exec = "bash containers/process-compose/webshot_pc.sh prodlike down";
    cwd = config.git.root;
  };

  tasks."webshot:prodlikeStatus" = {
    exec = "bash containers/process-compose/webshot_pc.sh prodlike status";
    cwd = config.git.root;
  };

  tasks."webshot:prodlikeLogs" = {
    exec = "bash containers/process-compose/webshot_pc.sh prodlike logs";
    cwd = config.git.root;
  };
