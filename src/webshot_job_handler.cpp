...
WebshotJobHandler::WebshotJobHandler(
...
us::yaml_config::Schema WebshotJobHandler::GetStaticConfigSchema()
{
    return us::yaml_config::MergeSchemas<server::handlers::HttpHandlerBase>(R"(
type: object
description: Webshot job handler static config
...
std::string WebshotJobHandler::HandleRequestThrow(
...
) const
{
    ...
    try {
        ...
        Uuid uuid;
        try {
            uuid = us::utils::BoostUuidFromString(uuidStr);
        } catch (const std::exception &) {
            return httpu::respondError(response, kBadRequest, "invalid uuid");
        }

        auto job = crud.findCrawlJob(uuid);
        if (!job)
            return httpu::respondError(response, kNotFound, "job not found");
        return httpu::respondJson(response, kOk, *job);
    } catch (const engine::WaitInterruptedException &) {
        throw;
    } catch (const std::exception &e) {
        LOG_ERROR() << fmt::format("Unhandled error in webshot_job_handler: {}", e.what());
