...
{
    const StsCredentials creds(makeValidXml());

    EXPECT_EQ(creds.accessKeyId.GetUnderlying(), std::string("AKIA_TEST_KEY"));
    EXPECT_EQ(creds.secretAccessKey.GetUnderlying(), std::string("SECRET_TEST_KEY"));
    EXPECT_EQ(creds.sessionToken.GetUnderlying(), std::string("TOKEN_TEST_VALUE"));
...
UTEST(S3StsClient, MissingTagThrows)
{
    ...
    EXPECT_THROW(StsCredentials creds(xml), std::runtime_error);
}
...
UTEST(S3StsClient, BuildsRequestWithExecutor)
{
    ...
    v1::detail::StsExecutor exec = [&](const std::string &url, const std::string &body,
    ...
    const auto creds = v1::detail::fetchStsWithExecutor(
        exec, endpoint, v1::s3v4::AccessKeyId{"AKIA_STATIC"}, v1::s3v4::SecretAccessKey{"SECRET"},
        "us-east-1", "arn:aws:iam::123456789012:role/TestRole", "session-name", R"({"allow":true})",
