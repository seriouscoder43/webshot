...
from conan.tools.files import apply_conandata_patches, copy, get, load
...
class WebshotUserverConan(ConanFile):
    name = "userver"
    ...
    def _use_sanitizers(self):
        return bool(self.conf.get("user.webshot:use_sanitizers") or False)
    ...
    def requirements(self):
        ...
        with_jemalloc = self.options.with_jemalloc and not self._use_sanitizers()
        if with_jemalloc:
    ...
    def validate(self):
        ...
        if (
            ...
            and self.dependencies["mongo-c-driver"].options.with_sasl != "cyrus"
        ):
            raise ConanInvalidConfiguration(
    ...
    def generate(self):
        ...
        if self._use_sanitizers():
            tool_ch.variables["USERVER_SANITIZE"] = "addr ub"
        ...
        jemalloc_enabled = self.options.with_jemalloc and not self._use_sanitizers()
        tool_ch.variables["USERVER_FEATURE_JEMALLOC"] = jemalloc_enabled
        ...
        tool_ch.variables["USERVER_FEATURE_GRPC_PROTOVALIDATE"] = (
            self.options.with_grpc_protovalidate
        )

        if self.options.with_grpc:
            tool_ch.variables["USERVER_GOOGLE_COMMON_PROTOS"] = (
                self.dependencies["googleapis"]
                .cpp_info.components["google_rpc_status_proto"]
                .resdirs[0]
            )

        if self.options.with_otlp:
            tool_ch.variables["USERVER_OPENTELEMETRY_PROTO"] = self.dependencies[
                "opentelemetry-proto"
    ...
    def package_info(self):
        # Expose userver to CMake via CMakeDeps config files
        self.cpp_info.set_property("cmake_find_mode", "config")
        cmake_dir = os.path.join("lib", "cmake", "userver")
        self.cpp_info.builddirs.append(cmake_dir)
        # Make userver CMake helpers
        # (userver_add_sql_library, userver_target_generate_chaotic, etc.)
        # available to consumers via CMakeDeps build modules.
        self.cpp_info.set_property(
            "cmake_build_modules",
            [
                os.path.join(cmake_dir, "UserverSql.cmake"),
                os.path.join(cmake_dir, "ChaoticGen.cmake"),
            ],
        )
